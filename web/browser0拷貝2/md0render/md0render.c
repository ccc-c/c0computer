...
void rerender(App* app) {
    ...
    // Clip content to area below nav bar
    SDL_Rect clip = {0, NAV_BAR_HEIGHT, WINDOW_WIDTH, WINDOW_HEIGHT - NAV_BAR_HEIGHT};
    ...
    SDL_RenderSetClipRect(app->ren, NULL);
...
static SDL_Rect clear_btn = {WINDOW_WIDTH - 75, 30, 30, 22};
static SDL_Rect url_bar_rect = {5, 30, WINDOW_WIDTH - 85, 22};
...
void draw_nav_bar(App* app) {
    ...
    // Clear button
    SDL_SetRenderDrawColor(app->ren, 200, 80, 80, 255);
    SDL_RenderFillRect(app->ren, &clear_btn);
    SDL_SetRenderDrawColor(app->ren, 160, 50, 50, 255);
    SDL_RenderDrawRect(app->ren, &clear_btn);
    
    SDL_Color white_c = {255, 255, 255, 255};
    SDL_Surface* clr_surf = TTF_RenderUTF8_Blended(app->font, "X", white_c);
    SDL_Texture* clr_tex = SDL_CreateTextureFromSurface(app->ren, clr_surf);
    SDL_Rect clr_dst = {clear_btn.x + 9, clear_btn.y + 3, clr_surf->w, clr_surf->h};
    SDL_RenderCopy(app->ren, clr_tex, NULL, &clr_dst);
    SDL_FreeSurface(clr_surf);
    SDL_DestroyTexture(clr_tex);
...
void url_bar_handle_key(App* app, SDL_Event* e) {
    ...
    if (sym == SDLK_a && (e->key.keysym.mod & (KMOD_GUI | KMOD_CTRL))) {
        // Select all = clear all (for URL bar simplicity)
        app->url_bar[0] = '\0';
        app->url_bar_cursor = 0;
        return;
    }
    if (sym == SDLK_x && (e->key.keysym.mod & (KMOD_GUI | KMOD_CTRL))) {
        // Cut: copy to clipboard then clear
        SDL_SetClipboardText(app->url_bar);
        app->url_bar[0] = '\0';
        app->url_bar_cursor = 0;
        return;
    }
